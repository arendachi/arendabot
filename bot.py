...
# ID или @username канала для автопостинга новых квартир.
# Задайте переменную CHANNEL_ID в Railway → Variables, например: @mychannel
# Если переменная не задана — автопостинг отключён, всё остальное работает как обычно.
...
def kb_main_menu(user_id: int):
    ...
    builder.button(text="🔢 Найти по номеру #ID", callback_data="search_by_id")
    if is_admin(user_id):
...
# ==================== ПОИСК ПО ID (для всех пользователей) ====================

@dp.callback_query(F.data == "search_by_id")
async def search_by_id_start(callback: CallbackQuery, state: FSMContext):
    await state.set_state(SearchByID.waiting_id)
    await callback.message.edit_text(
        "🔢 Введите номер квартиры (#ID)\n\n"
        "Например, если квартира называлась «Квартира #42» — напишите просто: 42"
    )
    await callback.answer()


@dp.message(SearchByID.waiting_id)
async def search_by_id_entered(message: Message, state: FSMContext):
    text = message.text.strip().lstrip("#")
    if not text.isdigit():
        await message.answer("⚠️ Введите только число, например: 42")
        return

    await state.clear()
    apt = db.get_apartment(int(text))

    if not apt:
        await message.answer(
            f"😔 Квартира с номером #{text} не найдена.\n"
            "Проверьте номер и попробуйте снова.",
            reply_markup=kb_main_menu(message.from_user.id),
        )
        return

    card = format_apartment_card(apt)
    if apt["photo_id"]:
        await message.answer_photo(apt["photo_id"], caption=card)
    else:
        await message.answer(card)

    await message.answer("Меню:", reply_markup=kb_main_menu(message.from_user.id))
...
async def finalize_add_apartment(message: Message, state: FSMContext, photo_id, edit: bool, user_id: int):
    ...
    # Подтверждение владельцу (с телефоном)
    confirm_text = (
    ...
    # Автопостинг в канал (без номера телефона)
    if CHANNEL_ID:
        try:
            apt = db.get_apartment(apt_id)
            channel_text = format_apartment_card(apt)
            if photo_id:
                await bot.send_photo(CHANNEL_ID, photo_id, caption=channel_text)
            else:
                await bot.send_message(CHANNEL_ID, channel_text)
        except Exception as e:
            logging.warning(f"Не удалось отправить в канал {CHANNEL_ID}: {e}")
            await message.answer(
                "⚠️ Квартира сохранена в базе, но не удалось опубликовать в канал.\n"
