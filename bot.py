...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("view_photos:"):
        listing_id = int(data.split(":", 1)[1])
        await handle_view_photos(update, context, listing_id)
        return

    if data.startswith("toggle_photos:"):
        listing_id = int(data.split(":", 1)[1])
        if update.effective_user.id not in config.ADMIN_IDS:
            return  # та же защита от подделки callback_data
        await handle_toggle_photos(update, context, listing_id)
        return
...
def kb_id_search_result(listing_id: int, is_admin: bool, has_photos: bool, photos_public: bool) -> InlineKeyboardMarkup:
    """
    Кнопки под карточкой при поиске по ID:
      - «Связаться с администратором и запросить фото» — видна всем пользователям.
      - «🖼 Посмотреть фото» — видна админу всегда (если фото есть), обычным пользователям —
        только если admin сделал фото публичными (photos_public=True).
      - «✏️ Редактировать владельца» и «📷 Добавить фото» — видны только админу.
      - «👁 Показать фото пользователям» / «🙈 Скрыть фото от пользователей» — переключатель
        видимости, видна только админу и только если фото есть.
    """
    ...
    if has_photos and (is_admin or photos_public):
        rows.append([InlineKeyboardButton("🖼 Посмотреть фото", callback_data=f"view_photos:{listing_id}")])
    ...
    if is_admin:
        ...
        if has_photos:
            toggle_label = "🙈 Скрыть фото от пользователей" if photos_public else "👁 Показать фото пользователям"
            rows.append([InlineKeyboardButton(toggle_label, callback_data=f"toggle_photos:{listing_id}")])
...
async def handle_id_search(update: Update, context: ContextTypes.DEFAULT_TYPE, listing_id: int):
    ...
    has_photos = db.count_listing_photos(listing_id) > 0
    photos_public = bool(row["photos_public"]) if "photos_public" in row.keys() else False

    text = _format_listing_card(row, show_owner=is_admin)
    reply_markup = kb_id_search_result(listing_id, is_admin, has_photos, photos_public)
...
async def handle_view_photos(update: Update, context: ContextTypes.DEFAULT_TYPE, listing_id: int):
    """
    Показывает сохранённые фотографии объявления. Доступ уже проверен на этапе показа кнопки
    (kb_id_search_result не добавляет эту кнопку, если у пользователя нет права её видеть),
    но проверяем повторно здесь — на случай подделки callback_data обычным пользователем.
    """
    query = update.callback_query
    ...
    photos_public = bool(row["photos_public"]) if "photos_public" in row.keys() else False
    if not is_admin and not photos_public:
        return  # защита от подделки callback_data: обычному пользователю фото не открыты

    file_ids = db.get_listing_photos(listing_id)
    if not file_ids:
        await query.message.reply_text("📷 У этого объявления пока нет фотографий.")
        return

    if len(file_ids) == 1:
        await context.bot.send_photo(chat_id=query.message.chat_id, photo=file_ids[0])
    else:
        media = [InputMediaPhoto(media=fid) for fid in file_ids]
        await context.bot.send_media_group(chat_id=query.message.chat_id, media=media)


async def handle_toggle_photos(update: Update, context: ContextTypes.DEFAULT_TYPE, listing_id: int):
    """
    Переключает видимость фотографий объявления для обычных пользователей.
    Доступно только админу. После переключения карточка перерисовывается с обновлённой кнопкой.
    """
    query = update.callback_query
    row = db.get_listing_by_id(listing_id)
    if not row:
    ...
    current_public = bool(row["photos_public"]) if "photos_public" in row.keys() else False
    new_public = not current_public
    db.set_photos_public(listing_id, new_public)

    status = "видны всем пользователям 👁" if new_public else "скрыты от пользователей 🙈"
    await query.message.reply_text(f"Фотографии объявления #{listing_id} теперь {status}.")

    # Перерисовываем исходную карточку с обновлённым набором кнопок
    updated_row = db.get_listing_by_id(listing_id)
    is_admin = update.effective_user.id in config.ADMIN_IDS
    has_photos = db.count_listing_photos(listing_id) > 0
    text = _format_listing_card(updated_row, show_owner=is_admin)
    reply_markup = kb_id_search_result(listing_id, is_admin, has_photos, new_public)
    try:
        await query.edit_message_text(text, reply_markup=reply_markup)
    except Exception:
        # Если исходное сообщение нельзя отредактировать (например, прошло много времени) —
        # отправляем карточку заново, чтобы админ не остался без обновлённых кнопок.
